////////////////////////////////////////////////////////////////////////////////////////
// Supporting code for blog post "C++ Coroutines: Understanding the Compiler
// Transform"
//
...
// By Lewis Baker
//
// This code sample shows the lowering of the following simple coroutine into
// equivalent non-coroutine C++ code.
//
//   task f(int x);
//   task g(int x) {
//     int fx = co_await f(x);
...
//
// This section contains definitions of the parts of <coroutine> header needed
// by the lowering and implementation below.
...
// add promise type for from_promise
...
struct __coroutine_state_with_promise : __coroutine_state {
  ...
  // later init, and we also should manually deconstruct it.
  union {
    Promise __promise;
  };
};
...
public:
  ...
  void destroy() const { state_->__destroy(state_); }

  bool done() const { return state_->__resume == nullptr; }
...
public:
  ...
  Promise &promise() const { return state_->__promise; }
...
public:
  ...
  operator=(const coroutine_handle &) noexcept = default;
  ...
  constexpr void *address() const noexcept {
    return const_cast<__coroutine_state *>(
        &__coroutine_state::__noop_coroutine);
  }
...
// Allow conditionally including the definitions of the methods of the task
// type.
#define ENABLE_TASK_DEFINITIONS
...
inline std::coroutine_handle<task::promise_type>
...
struct __g_state : __coroutine_state_with_promise<__g_promise_t> {
  __g_state(int &&x) : x(static_cast<int &&>(x)) {
    // Initialise the function-pointers used by
    // coroutine_handle::resume/destroy/done().
  ...
  ~__g_state() { this->__promise.~__g_promise_t(); }
...
__coroutine_state *__g_resume(__coroutine_state *s) {
  ...
  suspend_point_0: {
    ...
    //  int fx = co_await f(x);
    {
      state->__s1.__tmp2.construct_from([&] { return f(state->x); });
      destructor_guard tmp2_dtor{state->__s1.__tmp2};

      state->__s1.__tmp3.construct_from([&] {
        return static_cast<task &&>(state->__s1.__tmp2.get())
            .operator co_await();
      });
      ...
      if (!state->__s1.__tmp3.get().await_ready()) {
        ...
        auto h = state->__s1.__tmp3.get().await_suspend(
            std::coroutine_handle<__g_promise_t>::from_promise(
                state->__promise));

        // A coroutine suspends without exiting scopes - so cancel the
        // destructor-guards.
        tmp3_dtor.cancel();
        tmp2_dtor.cancel();
      ...
      // guards here without calling the destructors and then recreate them
      // after the `suspend_point_1` label.
      tmp3_dtor.cancel();
      tmp2_dtor.cancel();
    }
